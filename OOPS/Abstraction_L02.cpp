...
class SportsCar : public Car {
    public: 
        ...
        void shiftGear(int gear) {
            if(!isEngineOn) {
                cout << brand << " " << model << " : Engine is odff! cannot shift gear." << endl;
                return;
            }
            currentGear = gear;
            cout << brand << " " << model " : Shiftes to gear " << currentGear << endl;
...
/*so in every OOPS code or LLD code we need to define one main abstract interface(car) to define core methods that each of its type should have. and then create object(sportscar, normalcar) which inherits those methods and define functionality of it */